...
\frame{
  ...
  \makebox[\textwidth][c]{\begin{tikzpicture}[show background rectangle, background rectangle/.style={fill=black}]
    \node[opacity=0.15] (background) at (0,0){\includegraphics[width=150cm]{{general/background}.jpg}};
    \node[anchor=south] (LHC) at ($(background.north)-(0,5)$){\includegraphics[width=90cm, trim=0 150 0 100,clip]{{general/LHC}.jpg}};
  ...
  %  \node[anchor=north east] (logoCMS) at (-30,-40) {\includegraphics[height=5cm]{{/usr/share/texmf/tex/latex/beamer/base/themes/theme/images/CMS-Color}.pdf}};
    ...
    \input{CmsBox}
   % \input{photonBox}
    \node[opacity=0.12,anchor=east] (background) at ($(cmsBox.south east)-(3,2)$){\includegraphics[width=20cm]{{general/DeathStar2}.jpg}};
    \input{Subjects}
    \input{UgentBox}
